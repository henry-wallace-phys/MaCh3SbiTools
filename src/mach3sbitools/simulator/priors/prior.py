...
class Prior(torch.distributions.Distribution):
    ...
    def __init__(
        ...
        if cyclical_parameters:
            ...
            cyclical_mask = torch.zeros(n_params, dtype=torch.bool)
    ...
    def _build_nuisance_filter(
        ...
        if nuisance_patterns is None:
            ...
            return torch.ones(n_pars, dtype=torch.bool)
    ...
    def variance(self) -> torch.Tensor:
        ...
        consistent.
        """
        variance = torch.zeros(self.n_params)
    ...
    def sample(self, sample_shape=torch.Size([])) -> torch.Tensor:
        ...
        for mask_map in self._priors:
            if isinstance(mask_map.distribution, MultivariateNormal):
                ...
                remaining = torch.ones(sample_shape, dtype=torch.bool)
                out = torch.empty(
