...
class SBITrainerLegacy:
    ...
    def __init__(
        ...
        logger.warning("USING LEGACY TRAINER — this version is faster but lacks features like EMA. ")
        self.device = device
    ...
    def train(
        ...
        # FIX: Initialise best_state from the current model weights so it is
        # never None. Previously, best_state was set to None at the top of the
        # loop and only updated on improving epochs — non-improving epochs
        # returned None, silently discarding the previously saved best weights.
        ...
        with progress:
            for epoch in range(start_epoch, int(config.max_epochs + 1)):
                ...
                # ── Checkpoints ───────────────────────────────────────
                # FIX: Pass best_state in so _handle_checkpoints can update it
                # in-place rather than returning None on non-improving epochs.
                best_state, best_val_loss, epochs_no_improve = self._handle_checkpoints(
                    val_loss, best_val_loss, epochs_no_improve, density_estimator, config.save_path, best_state
                )
    ...
    def _handle_checkpoints(
        ...
        best_state: Optional[dict],  # FIX: accept current best_state so it is never lost
        ...
        if val_loss < best_val_loss:
            ...
            epochs_no_improve += 1
        # FIX: always return best_state (either updated or the previously passed-in
        # value) so the caller never receives None on non-improving epochs.
